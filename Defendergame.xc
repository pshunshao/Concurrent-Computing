...
void attackerAnt(chanend toVisualiser, chanend toController) {
  ...
  int currentDirection = RIGHT;                  //the current direction the attacker is moving
  int moveForbidden = 0;                     //the verdict of the controller if move is allowed
...
void controller(chanend fromAttacker, chanend fromUser) {
  unsigned int lastReportedUserAntPosition = (DEFENDER_STARTING_POSITION-1);      //position last reported by userAnt
  ...
  while (!gameEnded) {
    select {
      case fromAttacker :> attempt:
          if (attempt == LEFT){                                                                //1 = left, 0 = right
